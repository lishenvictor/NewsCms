...
//	}
	...
	private void parseBlogPage(String blogContent, String link, String type) {
		...
		if("1".equals(type)) {
			titleElements = doc.select(PropertiesUtil.getValue2("wangyititle")); // 获取新闻标题
		}else if("2".equals(type)){
			titleElements = doc.select(PropertiesUtil.getValue2("tengxuntitle")); // 获取新闻标题
		}else{
			titleElements = doc.select(PropertiesUtil.getValue2("souhutitle")); // 获取新闻标题
		}
		...
		if("1".equals(type)) {
			contentElements = doc.select(PropertiesUtil.getValue2("wangyicontent")); // 获取新闻内容
		}else if("2".equals(type)){
			contentElements = doc.select(PropertiesUtil.getValue2("tengxuncontent")); // 获取新闻内容
		}else{
			contentElements = doc.select(PropertiesUtil.getValue2("souhucontent")); // 获取新闻内容
		}
