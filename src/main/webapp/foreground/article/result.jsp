		  		...
		  		<c:otherwise>
		  			<c:forEach var="article" items="${articleList}">
		  				<li style="margin-bottom: 20px">
						  	<span class="title"><a href="${pageContext.request.contextPath}/article/${article.id }.html" target="_blank">${article.title }</a></span>
						  	<span class="summary">摘要:${article.content }...</span>
						  	<span class="link"><a href="${pageContext.request.contextPath}/article/${article.id }.html">http://www.lishenvictor.com/article/${article.id }.html</a>&nbsp;&nbsp;&nbsp;&nbsp;发布日期：${article.releaseDateStr }</span>
						  </li>
		  			</c:forEach>
		  		</c:otherwise>
		  	</c:choose>
